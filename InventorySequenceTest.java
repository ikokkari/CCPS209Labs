...
public class InventorySequenceTest {
    ...
    @Test public void testFirstFourteen() {
        int[] expected = {
                0, 1, 1, 0, 2, 2, 2, 0, 3, 2, 4, 1, 1, 0
        };
        assertArrayEquals(expected,
                InventorySequence.inventorySequence(0, expected.length));
    }

    @Test public void testFirstEightyFive() {
        // From OEIS A342585
        int[] expected = {
    ...
    @Test public void testWindowAtThousand() {
        int[] expected = {
                10, 19, 14, 12, 14, 13, 10, 5, 4, 4, 6, 7, 4, 2, 1, 1, 1, 0, 43, 38, 44, 31, 50, 36, 42,
                32, 34, 38, 35, 35
        };
        assertArrayEquals(expected,
                InventorySequence.inventorySequence(1000, 1000 + expected.length));
    }

    @Test public void testWindowAtTenThousand() {
        int[] expected = {
                17, 17, 15, 24, 20, 16, 16, 16, 11, 15, 14, 10, 14, 17, 16, 9, 9, 14, 13, 13, 17, 8, 6,
                16, 8, 8, 9, 3, 11, 7, 6, 11, 5, 7, 6, 9, 5, 8, 6, 8, 6, 7, 5, 5, 4, 2, 6, 5, 5, 4
        };
        assertArrayEquals(expected,
                InventorySequence.inventorySequence(10000, 10000 + expected.length));
    }

    @Test public void testWindowAtHundredThousand() {
        int[] expected = {
        ...
        assertArrayEquals(expected,
                InventorySequence.inventorySequence(100000, 100000 + expected.length));
    }

    @Test public void testMassOneMillion() {
        massTest(1_000_000, 1_100_000, 3241069703L);
    }

    @Test public void testMassTenMillion() {
        massTest(10_000_000, 10_100_000, 4068686297L);
    }

    @Test(timeout = 10000)
    public void testMassHundredMillion() {
        massTest(100_000_000, 100_100_000, 926087478L);
    }

    @Test(timeout = 20000)
    public void testMassTwoBillion() {
